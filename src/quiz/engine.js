...
import { markWrong, markCorrect, getWrongAnswerPool } from './wrong-review.js';

// Quiz engine — extracted from mishpacha-mega.html
...
if(G.filt==='wrong-review'){
  // Wrong-answer review pool: previously-wrong Qs sorted by recency × IMA weight.
  // Persisted across reloads via G.S.wrongSet (see src/quiz/wrong-review.js).
  G.pool=getWrongAnswerPool();
  G.qi=0;G.sel=null;G.ans=false;return;
}
if(G.filt==='weak'){
...
export async function runExplainOnCall(qIdx){
  ...
  try{
    ...
    G._exCache[qIdx]=txt;try{localStorage.setItem('mishpacha_ex',JSON.stringify(G._exCache));}catch(e){}
...
if(isOk(q,G.sel)){G.S.qOk++;srScore(G.pool[G.qi],true);
  // Wrong-answer review: a correct answer on a previously-wrong Q advances its
  // consecutive-correct streak; eviction happens at 2 in a row (see wrong-review.js).
  try{markCorrect(G.pool[G.qi]);}catch(e){/* non-fatal */}
}
else{G.S.qNo++;
  ...
  // Wrong-answer review: record the index so it can be surfaced later via the
  // "Review wrong (N)" filter pill (see src/quiz/wrong-review.js).
  try{markWrong(G.pool[G.qi]);}catch(e){/* non-fatal */}
...
export function endMockExam(){
  ...
  try{const hist=JSON.parse(localStorage.getItem('mishpacha_mock_hist')||'[]');
