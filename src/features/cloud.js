...
const readiness=est;
let uid=localStorage.getItem('mishpacha_uid');
if(!uid){uid='u'+Math.random().toString(36).slice(2,10);localStorage.setItem('mishpacha_uid',uid);}
...
const myUid=localStorage.getItem('mishpacha_uid')||'';
let html='';
...
const entry={type,text,ts:Date.now(),version:APP_VERSION,uid:localStorage.getItem('mishpacha_uid')||'anon'};
let fb=[];try{fb=JSON.parse(localStorage.getItem('mishpacha_fb_sent')||'[]');}catch(e){}
fb.push(entry);
...
export function _sbDeviceId(){let id=localStorage.getItem('mishpacha_devid');if(!id){id='dev_'+Math.random().toString(36).slice(2,12);localStorage.setItem('mishpacha_devid',id);}return id;}
...
export async function cloudRestore(){
  ...
  if(!id)return;
  try{
    const res=await fetch(SUPA_URL+'/rest/v1/mishpacha_backups?id=eq.'+encodeURIComponent(id)+'&select=data,updated_at',{
    ...
    if(ok){
      ...
      try{if(Array.isArray(row.data&&row.data._mockHist))localStorage.setItem('mishpacha_mock_hist',JSON.stringify(row.data._mockHist.slice(-20)));}catch(e){}
      try{if(Array.isArray(row.data&&row.data._sessions))localStorage.setItem('mishpacha_sessions',JSON.stringify(row.data._sessions.slice(-30)));}catch(e){}
