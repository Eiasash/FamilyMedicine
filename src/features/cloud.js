...
// Cloud sync, leaderboard, feedback, diagnostics — extracted from mishpacha-mega.html
...
// Migration: supabase/migrations/0003_leaderboard_upsert.sql
...
try{
  const res=await fetch(SUPA_URL+'/rest/v1/rpc/mishpacha_leaderboard_upsert',{
    method:'POST',
...
export function _sbDeviceId(){
  // When logged in, use the username so cloud backups follow the user across devices.
  const user=getCurrentUser();
  if(user)return 'user_'+user.username;
  let id=localStorage.getItem('mishpacha_devid');
  if(!id){id='dev_'+Math.random().toString(36).slice(2,12);localStorage.setItem('mishpacha_devid',id);}
  return id;
}
export async function cloudBackup(){
  ...
  try{
    ...
    // v1.21.12: include the user's Anthropic API key in the cloud backup so it
    // travels with their account across devices — sibling-paired with Geriatrics
    // v1.26.1 (security): _apikey REMOVED from the cloud-sync payload. backup_get/
    ...
    // below for backward compat. Sibling of Geri v10.64.158 / Pnimit v10.4.44.
...
export async function cloudRestore(){
  // Logged-in users auto-restore from their account row; guests are prompted.
  const user=getCurrentUser();
  ...
  try{
    ...
    const res=await fetch(SUPA_URL+'/rest/v1/rpc/backup_get',{
      method:'POST',
      headers:{'apikey':_SB_KEY,'Authorization':'Bearer '+_SB_KEY,'Content-Type':'application/json'},
      body:JSON.stringify({p_app:'mishpacha',p_id:id})
    });
 ...
 *
 * Extracted from cloudRestore() in v1.18.0 so the post-login auto-restore
...
export function applyRestorePayload(rowData) {
  ...
  try { if (Array.isArray(rowData._mockHist)) localStorage.setItem('mishpacha_mock_hist', JSON.stringify(rowData._mockHist.slice(-20))); } catch (e) {}
