...
//
// In-app Study Plan generator (Mishpacha slice).
...
//   - src/features/study_plan/syllabus_data.json (frozen Mishpacha slice)
//   - supabase/migrations/0002_study_plans.sql   (server-side schema + RPCs)
...
async function _handleGenerate() {
  ...
  try {
    const out = buildPlan({
      topics:        SYLLABUS.Mishpacha.topics,
...
function _buildICS(display) {
  ...
  const lines = [
    'BEGIN:VCALENDAR',
    'VERSION:2.0',
    'PRODID:-//Mishpacha Mega//Study Plan//EN',
    'CALSCALE:GREGORIAN',
    'METHOD:PUBLISH',
  ];
  ...
  display.weeks.forEach((w) => {
    ...
    const summary = `Mishpacha — שבוע ${w.idx} (${w.used_hours}h)`;
    const body = w.topics
    ...
    lines.push(
      'BEGIN:VEVENT',
      `UID:mishpacha-week-${w.idx}-${w.start_date}@familymedicine.local`,
      `DTSTAMP:${dtstamp}`,
  ...
  display.ramp_weeks.forEach((r) => {
    const summary = `Mishpacha — ${r.mock_label} (שבוע חזרה ${r.idx})`;
    const dtEndExclusive = _addDaysISO(r.start_date, 7);
    lines.push(
      'BEGIN:VEVENT',
      `UID:mishpacha-ramp-${r.idx}-${r.start_date}@familymedicine.local`,
      `DTSTAMP:${dtstamp}`,
  ...
  lines.push(
    'BEGIN:VEVENT',
    `UID:mishpacha-exam-${display.summary.exam_date}@familymedicine.local`,
    `DTSTAMP:${dtstamp}`,
    `DTSTART;VALUE=DATE:${_icsDate(display.summary.exam_date)}`,
    `DTEND;VALUE=DATE:${_icsDate(_addDaysISO(display.summary.exam_date, 1))}`,
    `SUMMARY:${_icsEscape('🎯 בחינת שלב א\' רפואת המשפחה')}`,
    `DESCRIPTION:${_icsEscape('בחינת שלב א\' רפואת המשפחה (P0062-2025)')}`,
    'END:VEVENT',
  );
...
function _handleExportICS() {
  ...
  a.href = url;
  a.download = `mishpacha-study-plan-${_state.display.summary.exam_date}.ics`;
