...
import { TOPICS, EXAM_FREQ, IMA_WEIGHTS, HARRISON_PDF_MAP } from '../core/constants.js';
import { sanitize, fmtT, safeJSONParse, heDir } from '../core/utils.js';
import { getDueQuestions, getDueCount, getWeakTopics, getStudyStreak, getTopicStats, isExamTrap, getChaptersDueForReading, getDrillTarget, buildDrillPool } from '../sr/spaced-repetition.js';
...
export function renderPriorityMatrix(){
  ...
  // IM-1 (2026-07-15): G.S.ts is never written anywhere; read live per-topic
  // stats from getTopicStats() (derived from G.S.sr) — the real source.
...
export function saveSessionSummary(){
  try{
    ...
    const hist=JSON.parse(localStorage.getItem('mishpacha_sessions')||'[]');
    hist.push(sess);if(hist.length>30)hist.splice(0,hist.length-30);
    localStorage.setItem('mishpacha_sessions',JSON.stringify(hist));
  }catch(e){}
}
// ===== FEATURE 5: Daily Study Plan =====
export function renderDailyPlan(){
if(!G.S.examDate&&!localStorage.getItem('mishpacha_exam_date'))return '';
const examDate=G.S.examDate||localStorage.getItem('mishpacha_exam_date')||'';
...
h+=planStep('2',`Read <b>${sanitize(w.name)}</b><span class="daily-plan-meta">${wPct}% accuracy · ${G.QZ.filter(q=>q.ti===w.i).length} questions</span>`,ref?planAction('read','data-action="goto-lib-goroll"','Open'):'');
...
try{const _mh=JSON.parse(localStorage.getItem('mishpacha_mock_hist')||'[]');const _last=_mh[_mh.length-1];const _wn=_last&&_last.wrongIdxs?_last.wrongIdxs.length:0;if(_wn>0){const _dt=new Date(_last.date).toLocaleDateString('en-GB',{day:'numeric',month:'short'});h+=planStep('5',`Replay <b>${_wn} wrong answers</b><span class="daily-plan-meta">mock on ${_dt}</span>`,planAction('replay','data-action="replay-last-mock-wrong"','Start'));}}catch(e){}
h+=`</div></div>`;
return h;
}
export function setExamDate(){
const d=prompt('Enter exam date (YYYY-MM-DD):',G.S.examDate||'');
if(d&&/^\d{4}-\d{2}-\d{2}$/.test(d)){G.S.examDate=d;localStorage.setItem('mishpacha_exam_date',d);G.save();G.render();}
}
...
export function calcEstScore(){
  ...
  // IM-1 (2026-07-15): G.S.ts is never written; use live getTopicStats() (G.S.sr).
  const tSt=getTopicStats();
