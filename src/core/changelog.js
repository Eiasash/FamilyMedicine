...
export const CHANGELOG={
  '1.26.19': [
    'content(2026-Jun): +150 questions — the complete IMA שלב א\' רפואת המשפחה קיץ 2026 board exam (18.06.2026), tag 2026-Jun. Corpus 1121 → 1271 (append-only, so all existing indices — incl. the citation-pilot pins — are unshifted). Questions transcribed verbatim from the official שאלון booklet via a sequential-marker parser (marker-aware option regex handling the FM inline "א .טקסט", Latin "ג Plavix", and digit "ג 10%" option forms); the 4 table/image questions (Q12 vitiligo→Addison lab panel, Q78 gestational-thrombocytopenia lab table, Q79 statin dyslipidemia, Q112 asthma reversibility PFT) were read via page-render vision, including 2 RTL-scrambled tables (Q78/Q79). Answer keys read via vision from מפתח תשובות הרצת פרסום 119 (post-appeal authoritative); c_accept on every Q (2 multi-answer: Q15 אד, Q114 גד). Every question carries its canonical מראה מקום in the explanation — the official FM source table maps each Q to Goroll 8e (chapter/page), Nelson (peds), AFP/AAFP rapid-evidence reviews, ADA/GOLD/GINA, and Israeli MOH/הר"י clinical guidelines. Full clinical explanations authored via the AI pipeline and topic-classified into the 27-topic FM taxonomy. Zero overlap with the existing corpus (token-Jaccard dedup, max <0.6). data/.corpus_manifest.json regenerated (total 1271 + per-topic). Count-lock + ALLOWED_TAGS + PAST_EXAM_TAGS + per-tag EXPECTED_COUNTS updated (questionsCountLock, regressionGuards, fractureRepair, citationCoveragePilot baseline). BUILD_HASH 1271q-v1.26.19; package/app/SW markers bumped 1.26.18 → 1.26.19.',
  ],
  '1.26.18': [
    'security: rotated the shared Toranot AI-proxy secret (x-api-secret header) after the 2026-07-13 weekly audit flagged it as hardcoded client-visible plaintext, identical across Geriatrics/InternalMedicine/FamilyMedicine and unchanged since introduction. Coordinated rotation across all three sibling apps + the Netlify-side proxy config, so the previous value is now invalid everywhere. Note: this invalidates anyone who scraped the old value, but does not change the underlying architecture — the new value is embedded in client code and repo docs the same way the old one was, since this is a public static-site repo with no server-authenticated user session. A real fix needs per-user tokens, not a shared static secret. No question content, answer keys, or explanations touched.',
  ],
  '1.26.17': [
  ...
  '1.26.7': [
    'feat(track): add a correct/wrong/טרם-נענה progress donut to the Track tab — the one stats visual it lacked (per AGENTS.md), now at parity with Pnimit (v10.4.48) and Shlav A. A conic-gradient donut (#16a34a correct / #dc2626 wrong / #e2e8f0 unanswered) computed from the FSRS spaced-rep store (G.S.sr) and the ACTIVE question set, surfacing TWO distinct accuracies: question-status (card currently "known", n>0) in the center and historical attempt accuracy (ok/tot across graded attempts) below it. Three correctness/a11y refinements folded in from the IM-sibling Codex review: (1) total + SR loop use the active set (dup/broken excluded, matching the render chokepoint in app.js/modes.js/more-view.js) so coverage can reach 100% — the FM bank has 5 dup questions; (2) a restored/legacy SR entry with tot/ok but no n is classified by hit-rate (ok>0), matching heatmap mastery, instead of reading as wrong; (3) the role=img aria-label leads with both center accuracies (דיוק שאלות + דיוק ניסיונות) so screen-reader users get them. Hebrew RTL-safe (dir=auto + unicode-bidi:plaintext). A give-up record (showAnswerHardFail, n:0 no tot) reads as wrong without inflating attempt accuracy. Does NOT duplicate the heatmap or priority matrix. src/ui/track-view.js _familyStatsDonutCard(); 9 tests in tests/trackStatsDonut.test.js. 0 data/questions.json changes; count-lock (1121) untouched. BUILD_HASH 1121q-v1.26.7; 1.26.6 to 1.26.7.'
  ],
  '1.26.6': [
    'fix(highyield): two residual nits in the v1.26.5 batch-2 high-yield Qs, plus the third-vendor audit wired in. (1) GERD item (ti 4) — option text read "PPR (Proton Pump Inhibitor)", a typo; the explanation and every sibling option use PPI. Corrected PPR to PPI. (2) HCV-screening item (ti 20) — stem said "גבר בן 55 ... נולד ב-1968" (age 55 vs born 1968 ≈ 58, internally inconsistent); recast to "גבר יליד 1968" and the explicit age dropped from the explanation (Codex/GPT nit, PR #151). CROSS-VENDOR confirmation: after both fixes, Gemini 2.5-pro (Google — independent of both Anthropic and GPT) blind-audited the two Qs and returned best=0 / matches-key / no-issues for each, closing the three-VENDOR loop (Anthropic generate+audit → GPT/Codex review → Google/Gemini confirm). data/questions.json + count-lock (1121) untouched; highyield bank stays 203. BUILD_HASH 1121q-v1.26.6; 1.26.5 to 1.26.6.'
  ],
  '1.26.5': [
    'content(highyield): +35 AI high-yield board MCQs into the additive data/highyield.json bank (168 to 203, tag AI-2026-hy) — same separate-bank design, so data/questions.json count-lock (1121) + BUILD_HASH count + cross-repo contract stay untouched. Generated FRESH by 5 Opus subagents (Toranot proxy secret unavailable) writing ORIGINAL vignettes, each warned off the pilot ambiguity traps (HFrEF ARNI-vs-MRA, diabetic-CKD RAS-vs-SGLT2, gout NSAID-vs-steroid, 2013 USPSTF lung-CT). THREE-MODEL gate: 40 candidates → 0 structural fails + 0 dups vs live 168+1121 → Opus blind audit 40/40 + Fable (different model) blind audit 40/40 → then CROSS-VENDOR Codex/GPT review caught 5 real P2 nuance issues that BOTH Anthropic models missed (cervical-screening age-30 vs 21-29 pathway; gono urethritis needs +doxycycline when chlamydia not excluded; nonsevere AOM allows observation; stage-2 HTN 150/96 warrants 2-drug start; gout explanation cited pre-ACR-2020 allopurinol-during-flare advice) → those 5 DROPPED, 35 triple-clean shipped. Demonstrates same-vendor agreement (Opus+Fable) is NOT independent validation — cross-vendor (GPT) review found what shared blind spots hid. ti re-derived by content. Audit doc: docs/AUDIT_HY_BATCH2_2026-06-11.md. No data/questions.json edits. BUILD_HASH 1121q-v1.26.5; 1.26.4 to 1.26.5.'
  ],
  '1.26.4': [
  ...
  '1.26.2': [
    'fix(account): sync the Anthropic API key to the account at save time (#353 sibling, Geri v10.64.160 / Pnimit v10.4.45). The v1.26.1 security fix removed _apikey from the cloud backup, which also severed the only WRITE path into app_users.api_key (the sync_api_key_from_backup trigger fed off backup writes) — a key saved or rotated after .1 stayed localStorage-only, so auth_login_user restored a stale or null key on the next device (the Codex P2 on #142). Now settings-save-api-key / settings-remove-api-key save locally FIRST (never network-blocked), then for logged-in users sync via the existing auth_set_api_key RPC (SECURITY DEFINER, re-auth required — password collected with window.prompt, the _handleChangePassword pattern). Cancel = device-only with explicit toast; RPC failure keeps the local save and warns; guests keep the old behavior. Empty key clears the account copy. Round-2 Codex P2s folded in: (a) the legacy backup _apikey restore in applyRestorePayload is now FILL-ONLY (a backup _apikey is pre-.1-stale by definition, so it never clobbers a present key); (b) the has-key settings state gained a logged-in-only sync-to-account button (settings-sync-api-key) so a key saved before this release can be pushed to the account without remove + re-enter. tests/apikeyAccountSync.test.js pins wiring + ordering + both P2 fixes + the .1 regression locks. BUILD_HASH 1121q-v1.26.2; 1.26.1 to 1.26.2.'
  ],
  '1.26.1': [
  ...
  '1.25.4': [
    '🧱 הכותרת נבנתה מחדש (flexbox) — אייקוני הכלים עברו משכבה אבסולוטית לשורת flex, כך שאינם חופפים עוד לכותרת "Mishpacha Mega" ולשורת התאריך. חותמת הגרסה חזרה אל מתחת לכותרת.'
  ],
  '1.25.3': [
    '🩺 תוקן דיוק רפואי בהסבר על SGLT2i ב-HFpEF — הובהר ש-EMPEROR-Preserved ו-DELIVER הפחיתו את התוצא המשולב בעיקר דרך הפחתת אשפוזים, ותמותה קרדיווסקולרית כתוצא עצמאי לא ירדה באופן מובהק (תוקן גם ניסוח עברי משובש).'
  ],
  '1.25.2': [
    '🩹 תוקן חפיפת חותמת הגרסה עם אייקוני הכותרת (הגדרות / מצב כהה / נר) בפינה הימנית — חותמת הגרסה הועברה לצד שמאל הריק של הכותרת ואינה נחתכת עוד על-ידי האייקונים.'
  ],
  '1.25.1': [
