...
export const CHANGELOG={
  '1.26.6': [
    'feat(track): add a correct/wrong/טרם-נענה progress donut to the Track tab — the one stats visual it lacked (per AGENTS.md), now at parity with Pnimit (v10.4.48) and Shlav A. A conic-gradient donut (#16a34a correct / #dc2626 wrong / #e2e8f0 unanswered) computed straight from the FSRS spaced-rep store (G.S.sr) and the loaded question array (G.QZ), surfacing TWO distinct accuracies: question-status (is the card currently "known", n>0) in the center and historical attempt accuracy (ok/tot across graded attempts) below it. Hebrew RTL-safe (dir="auto" + unicode-bidi:plaintext), role="img" with a percentage aria-label, and a coverage/answered/remaining footer. A give-up record (showAnswerHardFail writes n:0 with no tot) reads as a wrong question without inflating attempt accuracy. Deliberately does NOT duplicate the heatmap or priority matrix. src/ui/track-view.js _familyStatsDonutCard(); 7 tests in tests/trackStatsDonut.test.js (data correctness + give-up edge + source wiring guard). 0 data/questions.json changes; count-lock (1121) untouched. BUILD_HASH 1121q-v1.26.6; 1.26.5 to 1.26.6.'
  ],
  '1.26.5': [
