...
// State & storage — extracted from mishpacha-mega.html
...
G.S=safeJSONParse(LS,{ck:{},qOk:0,qNo:0,bk:{},notes:{},sr:{},fci:0,fcFlip:false,streak:0,lastDay:null,chat:[],studyMode:false,sp:{},spOpen:true,qnotes:{},gnotes:''});
...
G.save=function(){clearTimeout(G._saveTimer);G._saveTimer=setTimeout(()=>{
  ...
  try{
    let total=0;
    for(let k in localStorage)if(Object.prototype.hasOwnProperty.call(localStorage,k))total+=localStorage[k].length*2;
    if(total>4*1024*1024&&!window._lsWarnShown){
      ...
      localStorage.removeItem('mishpacha_ex');
      localStorage.removeItem('mishpacha_weekly');
    }
  }catch(e){}
  },150)};
...
idbSet('mishpacha_mega',JSON.parse(JSON.stringify(G.S))).catch(()=>{});
