...
G._dataPromise = (async function loadDataArrays() {
  ...
  // LCP fix (issue #25): distractors.json is ~1.7MB and is only consumed
  // in quiz-view.js:618 AFTER the user answers a question (`if(G.ans &&
  ...
  try {
    ...
    const _xp=safeJSONParse('mishpacha_pending_qs',[]);
    const _xc=safeJSONParse('mishpacha_custom_qs',[]);
    ...
    if (ct) {
      ct.innerHTML = '<div style="padding:20px;color:red;text-align:center">' +
        ...
        '<br><button id="mishpacha-retry-btn" style="margin-top:12px;padding:6px 14px;background:#0ea5e9;color:#fff;border:none;border-radius:8px;cursor:pointer;font-size:12px">Retry</button>' +
        '</div>';
      var _retryBtn = document.getElementById('mishpacha-retry-btn');
