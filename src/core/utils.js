...
// Utility functions — extracted from mishpacha-mega.html
...
// API key management
export function getApiKey(){return localStorage.getItem('mishpacha_apikey')||'';}
export function setApiKey(k){if(k)localStorage.setItem('mishpacha_apikey',k.trim());else localStorage.removeItem('mishpacha_apikey');}
...
export function remapExplanationLetters(text,shuf){
  ...
  //     and foreign-sound markers like "ג'נטיקה" (genetics).
  // Single regex with alternation prevents double-remap.
  // v1.21.8 — was missing form (3); ported from Geri v10.64.22 / IM v10.4.9.
