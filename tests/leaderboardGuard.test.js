...
describe('submitLeaderboardScore \u2014 threshold guard', () => {
  ...
  it('submits when answered >= threshold AND calcEstScore is a real number', async () => {
    ...
    // Post-migration 0003: write goes through SECURITY DEFINER RPC, not direct table POST.
    expect(url).toContain('/rest/v1/rpc/mishpacha_leaderboard_upsert');
  ...
  it('generates and caches a uid when none exists', async () => {
    ...
    expect(localStorage.getItem('mishpacha_uid')).toBeNull();
    await submitLeaderboardScore();
    const uid = localStorage.getItem('mishpacha_uid');
    expect(uid).toBeTruthy();
    expect(uid).toMatch(/^u[a-z0-9]+$/);
    // Second call reuses the same uid
    globalThis.fetch.mockClear();
    await submitLeaderboardScore();
    expect(localStorage.getItem('mishpacha_uid')).toBe(uid);
  });
});
