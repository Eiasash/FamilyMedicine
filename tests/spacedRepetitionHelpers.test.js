 ...
 *
 * Setup mirrors tests/srScore.test.js from the Pnimit twin — load
 * shared/fsrs.js into globalThis before importing spaced-repetition.js so
 * fsrs-bridge.js sees real FSRS functions at module-load time.
 */
...
describe('trackChapterRead + getChaptersDueForReading', () => {
  ...
  it('returns only chapters older than the day threshold, from the matching source', () => {
    const now = Date.now();
    G.S.chReads = {
      harrison_1: now - 40 * 86400000,
      harrison_2: now - 10 * 86400000,
      articles_1: now - 60 * 86400000,
    };
