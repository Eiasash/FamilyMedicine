...
describe('auth — localStorage state machine', () => {
  ...
  test('setAuthSession() persists profile and binds uid to username', async () => {
    ...
    expect(localStorage.getItem('mishpacha_uid')).toBe('danny');
  });
  ...
  test('getUserId() falls back to a stable random uid when not logged in', async () => {
    ...
    expect(localStorage.getItem('mishpacha_uid')).toBe(id1);
  });
  ...
  test('logout() clears profile and rotates uid+devid', async () => {
    ...
    localStorage.setItem('mishpacha_devid', 'dev_old');
    expect(getCurrentUser()).not.toBeNull();
    logout();
    expect(getCurrentUser()).toBeNull();
    expect(localStorage.getItem('mishpacha_uid')).not.toBe('danny');
    expect(getUserId()).toMatch(/^u[a-z0-9]{1,8}$/);
    expect(localStorage.getItem('mishpacha_devid')).not.toBe('dev_old');
    expect(localStorage.getItem('mishpacha_devid')).toMatch(/^dev_[a-z0-9]{1,10}$/);
  });

  test('getCurrentUser() rejects tampered profiles', async () => {
    const { getCurrentUser } = await import('../src/features/auth.js');
    // Garbage JSON
    localStorage.setItem('mishpacha_authUser', 'not json');
    expect(getCurrentUser()).toBeNull();
    // Wrong shape
    localStorage.setItem('mishpacha_authUser', JSON.stringify(null));
    expect(getCurrentUser()).toBeNull();
    localStorage.setItem('mishpacha_authUser', JSON.stringify({ x: 1 }));
    expect(getCurrentUser()).toBeNull();
    // Invalid username
    localStorage.setItem('mishpacha_authUser', JSON.stringify({ username: 'BadName!' }));
    expect(getCurrentUser()).toBeNull();
    // Valid
    localStorage.setItem('mishpacha_authUser', JSON.stringify({ username: 'good_one', displayName: null, loggedInAt: 1 }));
