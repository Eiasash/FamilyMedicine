 ...
 *
 * Known baseline: empty. Discovery scan (60ch + 2.5×, scoped to
 * PAST_EXAM_TAGS) found 0 hits across 1,061 records. Sibling Pnimit had
 * 1 hit (idx 544) at the same threshold; Mishpacha is cleaner. This guard
 * locks the clean state — if a future ingestion regresses, the test fails
 * and forces a fix before merge.
 */
const TIER2_TERMINAL_PUNCT = new Set([
  '?', '.', ':', '!', '"', "'", ')', ']', '״', '׳', '\u201D', '\u2019', '…',
]);
const TIER2_KNOWN_BASELINE = new Set([]);
