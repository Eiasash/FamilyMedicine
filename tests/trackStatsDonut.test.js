...
describe('track progress donut wiring (source guard)', () => {
  ...
  it('uses a conic-gradient donut with Hebrew RTL-safe attrs and an a11y label that includes both accuracies', () => {
    expect(SRC).toContain('conic-gradient(');
    expect(SRC).toContain('סטטיסטיקת התקדמות');
    expect(SRC).toContain('unicode-bidi:plaintext');
    expect(SRC).toMatch(/role="img" aria-label="דיוק שאלות \$\{accuracyText\}, דיוק ניסיונות/);
  });
