 ...
 *   1. FM corpus changes (questions added/removed/retagged)
...
const SECTION = 'Mishpacha'; // this repo's section name in Geri's syllabus
...
async function main() {
  ...
  if (section.total_questions_analyzed !== manifest.total_questions) {
    drifts.push(`total_questions_analyzed: Geri.${SECTION}=${section.total_questions_analyzed}, FM manifest=${manifest.total_questions}`);
  }
  ...
  for (const id of [...allIds].sort((a, b) => a - b)) {
    ...
    if (g !== m) {
      drifts.push(`topic ${id}: Geri.${SECTION}=${g ?? '(missing)'}, FM manifest=${m ?? '(missing)'}`);
    }
  }

  if (drifts.length === 0) {
    console.log(`OK: ${SECTION} section in Geri syllabus matches FM corpus manifest (${manifest.total_questions} questions, ${manifest.topics.length} topics).`);
    process.exit(0);
  }

  console.error(`DRIFT: Geri syllabus ${SECTION} section is out of sync with FM corpus manifest:`);
