...
async function extractExplanationAndSource(page) {
  ...
  let source = '';
  try {
    const body = page.locator('.quiz-feedback__body').first();
    if ((await body.count().catch(() => 0)) > 0) {
      explanation = (await body.innerText({ timeout: 800 })).trim().slice(0, 2500);
    }
    const src = page.locator('.quiz-source').first();
    if ((await src.count().catch(() => 0)) > 0) {
      source = (await src.innerText({ timeout: 500 })).trim().slice(0, 400);
    }
  } catch (_) { /* fall */ }
  // Fallback: if body+source both empty, try .card (v3 behavior) but only as backstop
...
const SYS_DOCTOR_JUDGE = `You are an experienced family-medicine attending grading a board-exam question and the app's answer key. The question is from an Israeli family-medicine board prep app. Your job: validate the APP's claimed correct answer against board-level family-medicine evidence (Goroll 8e, Nelson 22e, AFP, Israeli MOH/הר"י). The AI's prior pick is context only — you are NOT adjudicating "AI vs app", you are validating the APP's stated key.
...
Validate the APP's claimed answer ${appLetter} against board-level family-medicine evidence.`;
  ...
  await sleep(rand(800, 1700));
  // Leaderboard hook: every 25th successful answer per worker, trigger
  // window.showLeaderboard() which internally calls submitLeaderboardScore.
  // The RPC needs ≥20 answered + a non-null calcEstScore — we hit both
  // at this point. No-op if window.showLeaderboard not bound (Geri-style).
  ...
  if (log._lbCount % 25 === 0) {
    ...
    } catch (_) { /* swallow — not critical to bot run */ }
