...
## Current state (v1.26.22, 19/07/26)

- **v1.26.19 content(2026-Jun)**: +150 questions — the complete IMA שלב א' רפואת המשפחה קיץ 2026 board exam (18.06.2026), tag `2026-Jun`. `data/questions.json` 1121 → 1271 (append-only, so all existing indices incl. citation-pilot pins are unshifted). Verbatim from the official שאלון via a sequential-marker parser (FM inline "א .טקסט" / Latin "ג Plavix" / digit "ג 10%" option forms); 4 table/image questions (Q12 vitiligo→Addison panel, Q78 gestational-thrombocytopenia, Q79 statin dyslipidemia, Q112 asthma-reversibility PFT) read via page-render vision, incl. 2 RTL-scrambled tables. Keys via vision from מפתח תשובות הרצת פרסום 119 (post-appeal); `c_accept` on every Q (2 multi-answer). Every Q carries its canonical מראה מקום in the explanation — Goroll 8e (ch/page), Nelson (peds), AFP/AAFP rapid-evidence reviews, ADA/GOLD/GINA, and Israeli MOH/הר"י guidelines. AI-pipeline explanations, topic-classified (27-topic taxonomy). Zero corpus overlap; corpus_manifest regenerated (1271). BUILD_HASH 1271q-v1.26.19; quartet bumped 1.26.18 → 1.26.19.
...
- **+203 AI high-yield Qs** in a separate `data/highyield.json` bank (tag `AI-2026-hy`) loaded additively + labeled "AI — High-Yield" in-app; `data/questions.json` count-lock + BUILD_HASH count + cross-repo manifest/syllabus contract tracked separately. Audited blind (opus); 1.26.0 shipped 133 (57 flags held), v1.26.3 added 35 survivors from a 69-new pilot (34 held — 26 ambiguity flags + 8 confident blind disagreements; see `docs/AUDIT_HY_PILOT_2026-06-11.md`).
- **1271 Qs** total — 1100 across 8 exam sessions (2020=150, 2021-Jun=150, 2022-Jun=150, 2023-Jun=150, 2024-May=100, 2024-Sep=100, 2025-Jun=150, 2026-Jun=150) + 111 `FM-Core` curated textbook Qs
- All 8 sessions CONFIRMED Family Medicine content (fork-bug remediated in v1.3.0 — see CHANGELOG)
- **27 topics** (`ti` range 0-26), **47 drugs**, **0 flashcards** (unused so far)
- **81 test files** under `tests/` (v1.21.15 added 2 SW regression guards: HTML_URLS *.js + CSS_URLS on-disk). Pnimit regression guards ported + Mishpacha-specific guards added + AFP topic-map / FSRS boundary tests (v1.21.1) + R2 deep coverage (quiz-engine multi-tag, study-plan DST/calendar, sw manifest, IDB mock, bidi numerics, mutation resistance — v1.21.2) + apiKeyLoginRestore covering v1.21.12-14 cloud sync + 3 P0 chaos crash fixes (v1.21.13: toLowerCase undefined, flashcards `f` undefined bounds-check, startTimedQ G-binding)
...
- localStorage `mishpacha_mega`, SW cache `mishpacha-v1.26.22`
