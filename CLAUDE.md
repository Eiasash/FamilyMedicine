...
## Current state (v1.26.2, 09/06/26)
- **+133 AI high-yield Qs** in a separate `data/highyield.json` bank (tag `AI-2026-hy`) loaded additively + labeled "AI — High-Yield" in-app; `data/questions.json` UNCHANGED (1121) so count-lock + BUILD_HASH count + cross-repo manifest/syllabus contract untouched. Audited blind by `scripts/audit_keys_blind.mjs` (opus); 57 flags held out for review.
...
- localStorage `mishpacha_mega`, SW cache `mishpacha-v1.26.2`
