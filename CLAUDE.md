...
## Current state (v1.26.6, 12/06/26)
...
- localStorage `mishpacha_mega`, SW cache `mishpacha-v1.26.6`
- **v1.26.6 Track progress donut** — correct/wrong/טרם-נענה conic-gradient donut on the Track tab (parity with Pnimit/Shlav; the one stats visual it lacked per AGENTS.md). Reads `G.S.sr`/`G.QZ`; shows question-status vs historical attempt accuracy. No heatmap/priority-matrix duplication. `src/ui/track-view.js` `_familyStatsDonutCard()`; 7 tests in `tests/trackStatsDonut.test.js`.
